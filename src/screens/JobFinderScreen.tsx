  ...
  const fetchJobs = async () => {
    try {
      ...
      const transformedJobs: Job[] = jobsArray.map((job: any) => ({
        ...
        company:
          job.company || job.company_name || job.employer || "Unknown Company",
        salary:
          job.salary || job.salary_range || job.pay || "Salary not specified",
        location:
          job.location || job.city || job.address || "Location not specified",
      }));
